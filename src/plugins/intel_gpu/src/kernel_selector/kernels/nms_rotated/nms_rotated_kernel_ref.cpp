...
static inline size_t GetOptimalLocalClassSize(std::vector<size_t> gws, const EngineInfo& info) {
    constexpr size_t optimal_values[] = {256, 227, 224, 192, 160, 128, 96, 64, 32, 16, 8, 7, 6, 5, 4, 2, 1};
...
NMSRotatedKernelRef::DispatchData SetDefault(const nms_rotated_params& params, int idx) {
    NMSRotatedKernelRef::DispatchData dispatchData;
...
Datatype NMSRotatedKernelRef::GetAccumulatorType(const nms_rotated_params& params) const {
...
ParamsKey NMSRotatedKernelRef::GetSupportedKey() const {
    ParamsKey k;
...
JitConstants NMSRotatedKernelRef::GetJitConstants(const nms_rotated_params& params) const {
    ...
    jit.AddConstants({MakeJitConstant("SORT_RESULT_DESCENDING", params.sort_result_descending),
                      MakeJitConstant("BOX_ENCODING", static_cast<int>(BoxEncodingType::BOX_ENCODING_CENTER))});

    const auto output_num = params.outputs[0].Batch().v;
    jit.AddConstant(MakeJitConstant("OUTPUT_NUM", output_num));

//    if (params.num_select_per_class_type == base_params::ArgType::Input) {
        jit.AddConstant(MakeJitConstant("NUM_SELECT_PER_CLASS_TYPE", GetInputTypeStr(2)));
...
//    if (params.iou_threshold_type == base_params::ArgType::Input) {
        jit.AddConstant(MakeJitConstant("IOU_THRESHOLD_TYPE", GetInputTypeStr(3)));
...
//    if (params.score_threshold_type == base_params::ArgType::Input) {
        jit.AddConstant(MakeJitConstant("SCORE_THRESHOLD_TYPE", GetInputTypeStr(4)));
...
//    }
        ...
        jit.AddConstant(MakeJitConstant("SECOND_OUTPUT_TYPE", GetInputTypeStr(1)));
        jit.AddConstant(MakeJitConstant("TO_SECOND_OUTPUT_TYPE", GetToInputTypeStr(1)));
        jit.AddConstant(MakeJitConstant("SECOND_OUTPUT_GET_INDEX", GetToInputIndexStr(1)));

        jit.AddConstant(MakeJitConstant("THIRD_OUTPUT_TYPE", GetInputTypeStr(2)));
        jit.AddConstant(MakeJitConstant("TO_THIRD_OUTPUT_TYPE", GetToInputTypeStr(2)));
        jit.AddConstant(MakeJitConstant("THIRD_OUTPUT_GET_INDEX", GetToInputIndexStr(2)));
...
bool NMSRotatedKernelRef::Validate(const Params& p, const optional_params& o) const {
    if (p.GetType() != KernelType::NMS_ROTATED || o.GetType() != KernelType::NMS_ROTATED) {
        return false;
    }

    const nms_rotated_params& params = static_cast<const nms_rotated_params&>(p);
 ...
 *
 */
void NMSRotatedKernelRef::SetKernelArguments(const nms_rotated_params& params,
...
KernelsData NMSRotatedKernelRef::GetKernelsData(const Params& params, const optional_params& options) const {
    if (!Validate(params, options)) {
        return {};
    }

    constexpr size_t kKernelsNum = 4;
    KernelData kd = KernelData::Default<nms_rotated_params>(params, kKernelsNum);
    const nms_rotated_params& orgParams = static_cast<const nms_rotated_params&>(params);

    // Assign internel buffer
    constexpr size_t intermediate_bytes = 12;   // struct size of SortedBoxInfo/BoxInfo in nms_rotated_gpu_ref.cl
...
KernelsPriority NMSRotatedKernelRef::GetKernelsPriority(const Params& /*params*/, const optional_params& /*options*/) const {
