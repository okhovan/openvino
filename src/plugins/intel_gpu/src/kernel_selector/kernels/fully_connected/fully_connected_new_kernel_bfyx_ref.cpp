...
JitConstants FullyConnectedNew_bfyx_Ref::GetJitConstants(const fully_connected_params& params,
    ...
    const auto output_rank = params.output_shape.rank().get_length();
    jit.AddConstant(MakeJitConstant("OUTPUT_RANK", output_rank));

    const auto input_rank = params.input_shape.rank().get_length();
    jit.AddConstant(MakeJitConstant("INPUT_RANK", input_rank));

    const auto weight_rank = params.weights_shape.rank().get_length();
    jit.AddConstant(MakeJitConstant("WEIGHT_RANK", weight_rank));
...
bool FullyConnectedNew_bfyx_Ref::Validate(const Params& params, const optional_params& options) const {
    ...
    if (fc_params.GetType() != KernelType::FULLY_CONNECTED) {
        return false;
    }
