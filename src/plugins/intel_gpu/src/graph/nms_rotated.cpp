...
namespace cldnn {
GPU_DEFINE_PRIMITIVE_TYPE_ID(nms_rotated)

layout nms_rotated_inst::calc_output_layout(nms_rotated_node const& node, kernel_impl_params const& impl_param) {
    const auto desc = impl_param.typed_desc<nms_rotated>();
    const auto output_type = desc->output_data_types[0].value_or(data_types::i32);
    const auto output_size = tensor(batch(desc->max_number_of_boxes), feature(3));
    return layout(output_type, impl_param.get_input_layout().format, output_size);
}

/*
template<typename ShapeType>
std::vector<layout> nms_rotated_inst::calc_output_layouts(nms_rotated_node const& node
, const kernel_impl_params& impl_param) {
    std::vector<layout> layouts;

    auto desc = impl_param.typed_desc<nms_rotated>();
...
template std::vector<layout> nms_rotated_inst::calc_output_layouts<ov::PartialShape>(nms_rotated_node const& node,
                                                                                             const kernel_impl_params& impl_param);
*/

std::string nms_rotated_inst::to_string(nms_rotated_node const& node) {
    const auto desc = node.get_primitive();
    const auto node_info = node.desc_to_json();

    json_composite info;
    info.add("center point box", desc->sort_result_descending);
    info.add("center point box", desc->output_data_type);
    info.add("center point box", desc->clockwise);
    node_info->add("NMS Rotated info", info);
